class ReportsController < ApplicationController
  ...
  def sparc_request_report
    @requests = current_user.eligible_requests.filtered_for_index(report_params[:term], report_params[:status], report_params[:sort_by], report_params[:sort_order]).eager_load(:requester, specimen_requests: [:labs, :groups_source, :group]).preload(:primary_pi, :specimen_requests, protocol: { project_roles: :identity }, additional_services: [:service, :sub_service_request])
